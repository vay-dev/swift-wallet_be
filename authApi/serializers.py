...
class UserProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserProfile
        fields = ['display_picture', 'bio', 'date_of_birth', 'address', 'city', 'country']
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CustomUser
        fields = ['id', 'phone_number', 'account_number', 'full_name', 'email', 'is_verified', 'date_joined', 'profile']
        read_only_fields = ['id', 'account_number', 'is_verified', 'date_joined']
...
class AccountNumberChangeSerializer(serializers.Serializer):
    ...
    def validate_new_account_number(self, value):
        if CustomUser.objects.filter(account_number=value).exists():
            raise serializers.ValidationError("This account number is already taken.")
        return value
